...
* 01.1_run_glm_traj.R runs 01_make_glm_traj.R to create the panel data trajectories with threat level, arrest type, and conviction status by state and AOR using raw denominator data and the cleaned arrest data. Expected output: data/processed/glm_trajectory_type_threatlevel_convicted_ACS_MPI.csv, data/processed/glm_trajectory_type_threatlevel_convicted_ACS_MPI_AOR.csv. Time to run: 1 minute.
