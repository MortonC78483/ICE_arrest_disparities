...
trajectory %>%
  filter(region != "EUCA") %>%
  ungroup() %>%
  filter(window == 0 & threat_level == 0 & convicted == 0 & method == "CA") %>%
...
Remove duplicates
```{r}
trajectory <- read_csv("../data/processed/glm_trajectory_type_threatlevel_convicted_ACS_MPI_AOR_no_duplicates.csv")
